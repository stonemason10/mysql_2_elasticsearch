...
exports.pull = function(pool, table, sqlPhrase, filter_map, exception_handler, callback) {
  pool.getConnection(function(err, connection) {
    ...
    connection.query($sql.query, function(err, result) {
      if (err || !result) {
        ...
        let resContent = String(JSON.stringify( result ));
        console.log('数据拉取完毕！\n开始转换数据格式...');
  ...
  let json2bulk = function(originData, filter_map, exception_handler) {
    // 去除空格和制表符
    // let dataStr = String(originData).replace(/(\s+|\t+|\n+|\r+)/gi, '');

    // 给每一条数据添加配置信息
    let dataObj = JSON.parse(originData);

    let n = 0;
    while (n < dataObj.length) {
      let cur_id = dataObj[n].id;
    ...
    // 对已定义的异常数据进行批处理
    let resData = JSON.stringify(dataObj);

    if (exception_handler && Object.keys(exception_handler).length > 0) {
      let excHandlers = [];
      for (let field in exception_handler) {
        for (let i=0;i<exception_handler[field].length;i++) {
          let valPatt = exception_handler[field][i].match;
          let valExs = exception_handler[field][i].writeAs;
      ...
      for (let j=0;j<dataObj.length;j++) {
        if (!dataObj[j].delete && !dataObj[j].create) {
          for (let k=0;k<excHandlers.length;k++) {
    ...
    if (filter_map && filter_map.length > 0) {
      for (let i=0;i<filter_map.length;i++) {
        let compile_field = filter_map[i].replace(/(\_|\-)/gi, '\\$1');
        let patt_str = '/\"' + compile_field + '\"\:(\"[^\"]+\"|null|\d+|\"\")(\,)?/gi';
    ...
    let targetFile = path.join(__dirname, '../lib/', table + '.bulk.json');
...
exports.push = function(client, es_config, callback) {
  ...
  fs.readFile(path.join(__dirname, '../lib/', es_config.src_table + '.bulk.json'), 'utf8', function(err, bulkChunk) {
    ...
    let bulkJson = JSON.parse(bulkChunk);
    let chunkSize = !es_config.chunkSize ? 5000 : es_config.chunkSize;

    if (bulkJson.length > chunkSize*3) {
      let chunkNum = Math.ceil(bulkJson.length / (chunkSize*3));
